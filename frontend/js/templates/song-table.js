...
function songRowTemplate(song, index, options = {}) {
    ...
    if (searchable && hidePlayBtn) {
        ...
        const addBtnTitle = isInPlaylist ? '已添加到列表' : '添加到列表';
        const addBtnText = isInPlaylist ? '✓' : '+';
        indexContent = `
        ...
        // 其他页面：播放时显示音波动画
        indexContent = '<span class="playing-indicator"><span></span><span></span><span></span><span></span></span>';
    } else {
        // 其他页面：未播放时显示序号
...
function songTableTemplate(songs, options = {}) {
    ...
    let indexHeader = '';
    if (searchable && hidePlayBtn) {
        indexHeader = '<button class="add-all-header-btn" onclick="handleAddAllToPlaylist()" title="添加当前页所有歌曲到列表">当前页+</button>';
    }
...
function createSongRow(song, index, options = {}) {
    ...
    let actionButtons = '';
    let showActionColumn = showActions;
    
    if (showActions) {
        if (searchable) {
            // 搜索页面：添加按钮移到序号列
            showActionColumn = false;
        } else if (removable && !favoritable) {
            actionButtons = '<button class="action-btn remove-btn" title="移除">×</button>';
        } else if (favoritable && !hidePlayBtn) {
            const favBtnClass = isFavorite ? 'action-btn favorite-btn favorited' : 'action-btn favorite-btn';
            const favBtnTitle = isFavorite ? '取消收藏' : '收藏';
            
            actionButtons = `
                <button class="action-btn play-btn" title="播放">▶</button>
                <button class="${favBtnClass}" title="${favBtnTitle}">
                    <svg viewBox="0 0 24 24" width="18" height="18" fill="${isFavorite ? '#c62f2f' : 'none'}" stroke="${isFavorite ? '#c62f2f' : 'currentColor'}" stroke-width="2">
                        <path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/>
                    </svg>
                </button>
            `;
    ...
    const actionCell = showActionColumn ? `<td class="col-action">${actionButtons}</td>` : '';
